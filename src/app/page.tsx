...
import { filterableFields, SiteConfigSkeleton } from "@/lib/types";
...
export default async function Home({
    ...
    let allFilters =
        await redisClient.get<Record<string, string[]>>("all-filters");

    if (!allFilters) {
        console.log("Cache miss for all-filters");
        let pageCount = 0;
        while (allResults.items.length < allResults.total || pageCount === 20) {
    ...
    let siteConfig =
        await redisClient.get<Entry<SiteConfigSkeleton, undefined, string>>(
            SITE_KEY
        );
